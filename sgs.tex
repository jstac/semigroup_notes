...
Also, given $R \subset \RR$ and normed linear space $F$,
%
\begin{itemize}
    \item $C_0(R, F)$ is the set of continuous functions from $R$ to $F$
        and
    \item $C_1(R, F)$ is the set of continuously differentiable functions
        from $R$ to $F$.
\end{itemize}
%
...
We recall some facts concerning calculus for Banach space valued
functions. To this end, let $u$ be an element of $C_0(\RR_+, E)$. If, for some $t > 0$,
the limit
%
\begin{equation*}
    \dot u(t)
...
\end{equation}
%
where the left hand side is an ordinary Riemann integral for
...
Let $(A, \dD(A))$ be the generator of a $C_0$-semigroup $(S_t)$ on $E$. Since
$A$ is closed linear operator, we define the \navy{Resolvent} of $(S_t)$ by
$$R_{\lambda}(A)=(\lambda I-A)^{-1}$$ where $R_{\lambda}(A): E\rightarrow\dD(A)$
and $\lambda\in\rho(A)$. Thus, $R_{\lambda}(A) \in\mathcal{L}(E)$.
