...
Let's begin by recalling some basic definitions and fixing terminology.
...
\begin{itemize}
    ...
    \item $\| \cdot \|$ denotes either the norm on $E$ or the operator norm on
        $\lL(E)$, depending on context. 
\end{itemize}

Given $R \subset \RR$,
%
\begin{itemize}
    \item $C_0(R, E)$ is the set of continuous functions from $R$ to $E$
        and
    \item $C_1(R, E)$ is the set of continuously differentiable functions
        from $R$ to $E$.
\end{itemize}
%
The notion of derivative used in the last definition is discussed below.

A (not necessarily bounded) linear operator $A$ from $\dD(A) \subset
E$ into $E$ is called \navy{closed} if, for each sequence
$(u_n) \subset \dD(A)$ such that $u_n \to u$ for some $u \in E$ and $Au_n \to y$
for some $y \in E$, we have $u \in \dD(A)$ and, in addition, $y = Au$.

Let $A \colon \dD(A) \to E$ be a closed linear operator and let $I$ be the
identity map on $E$.  The set $\rho(A)$ of all $\lambda \in \CC$ such that
$\lambda - A \coloneq \lambda I - A$ is a bijection from $\dD(A)$ to $E$ is called the
\navy{resolvent set} of $A$.  Its complement $\CC \setminus \rho(A)$ is called
the \navy{spectrum}.  Given $\lambda \in \rho(A)$, the \navy{resolvent operator}
of $A$ is defined by
%
\begin{equation*}
    R_\lambda (A) u = (\lambda - A)^{-1} u
    \qquad (u \in E).
\end{equation*}
...
concepts to an abstract Banach space $E$ is to view $t \mapsto x(t)$ as a
differentiable function from $\RR_+$ to $E$ and $A$ is a \emph{bounded} linear
...
\subsection{Calculus}\label{ss:calc}

We recall some facts concerning calculus for functions mapping a subset of the
real line into a Banach space. To this end, let $u$ be an element of $C_0(\RR_+,
E)$. If, for some $t > 0$, the limit
%
\begin{equation*}
    \dot u(t)
...
the \navy{derivative} of $u$ at $t$.  In the case where $t=0$ we 
replace $h \to 0$ with $h \downarrow 0$.
We say that $u$ is \navy{differentiable} on $\RR_+$ if $u$ is differentiable at
$t$ for all $t \in \RR_+$.
We say that $u$ is \navy{continuously differentiable} on $\RR_+$ and write $u
\in C_1(\RR_+, E)$ if, in
addition, $\dot u \in C_0(\RR_+, E)$. 

Given $u \in C_0(\RR_+, E)$, the (Riemann) \navy{integral} of $u$ over $[a,b]$ is the unique point
$I \in E$ such that
%
...
\end{equation}
%
where the left hand side is an ordinary Riemann integral for real-valued
functions.  We write $I = \int_a^b u(s) \diff s$, so that $\int_a^b \inner{u(s),
u'} \diff s = \inner{\int_a^b u(s) \diff s, u'}$ for all $u' \in E$.  A proof of
existence and uniqueness, as well as the properties listed below, can be found
in Section~5.1.2 of \cite{buhler2018functional}.

\begin{lemma}\label{l:riemann}
    If $u \in C_0(\RR_+, E)$ and $a, b, c \in \RR_+$ with $a \leq b \leq c$, then
    %
    \begin{enumerate}
        \item $\int_a^c u(s) \diff s 
            = \int_a^b u(s) \diff s + \int_b^c u(s) \diff s$,
        \item $\int_a^b u(s + r) \diff s = \int_{a+r}^{b+r} u(s)
            \diff s$ whenever $r \geq 0$,
    ...
    \end{enumerate}
    %
    If, in addition, $u$ is continuously differentiable on $\RR_+$ with
...
\coloneq \me^a \coloneq \sum_{n=0}^\infty a^n/(n!)$.  Analogously, the \navy{exponential}
of $A \in \lL(E)$ is given by
%
\begin{equation}\label{eq:expfun} 
    \exp(A) 
    \coloneq \me^A \coloneq \sum_{n=0}^\infty \frac{A^n}{n!}.
\end{equation}
%
...
\begin{lemma}\label{l:expcom2}
    Given $A \in \lL(E)$, the function $\phi(t) \coloneq \exp(tA)$ is
...
\begin{proof}
    Fix $A \in \lL(E)$. Applying (ii) of Lemma~\ref{l:expcom} yields
    %
    \begin{equation*}
    ...
    0$ gives $\dot \phi(t) = \phi(t) A $.  A small variation on the argument 
    shows that $\dot \phi(t) = A \phi(t)$ also holds.
\end{proof}



Lemma~\ref{l:riemann} and Lemma~\ref{l:expcom2} together yield
%
\begin{equation}\label{eq:ftcexp}
    \exp(t A) 
     =  \exp(s A) + A \int_s^t \exp(r A) \diff r \quad \text{for all } s \leq t.
\end{equation}
%
...
Fix $A \in \lL(E)$ and consider the \navy{initial value problem} (IVP) 
%
\begin{equation}\label{eq:ivp}
    \dot u(t) = A u(t)
    \quad \text{for all } t \geq 0 
    \text{ with } u(0) = u_0 \in E \text{ given}.     
\end{equation}
%
A solution to this problem is a differentiable $u \in C_0(\RR_+, E)$ that obeys
\eqref{eq:ivp}.

\begin{proposition}\label{p:expsol}
    If $A \in \lL(E)$, then $u$ defined by $u(t) = \exp(tA) u_0$ is
    the unique solution to the IVP \eqref{eq:ivp}.
\end{proposition}
...
an \navy{algebraic operator semigroup (AO semigroup)} if 
...
is called a \navy{$C_0$-semigroup}.  Given $u \in E$, the function $t \mapsto S_t u$ is
...
\begin{lemma}\label{l:ubfc}
    ...
    %
    \begin{enumerate}
        \item $\sup_{t \leq \delta} \| S_t \| < \infty$ for all $\delta > 0$.
        \item there exists an $a \in \RR$ and $M \geq 1$ such that $\|S_t \|
            \leq M \exp(t a)$ for all $t \geq 0$.
    \end{enumerate}
\end{lemma}

\begin{proof}
    Regarding (i), we first claim there exists an $\epsilon > 0$ such that $
    ...
    The proof of (ii) follows from (i).  For details see, for example,
    Theorem~1.2.8 of \cite{applebaum2019semigroups}.
\end{proof}
...
We list some examples of semigroups acting on Banach spaces.


\subsubsection{Left-Shift Semigroups}

Let $C_V(\RR_+, \RR)$ be the set of all continuous real-valued functions $f$ on
$\RR_+$ with $f(x) \to 0$ as $x \to \infty$.  The set $C_V(\RR_+, \RR)$ is paired
with the supremum norm.  Consider the \navy{left
translation semigroup} given by $(S_t f)(x) = f(x + t)$.

\begin{lemma}\label{l:ltsemi}
    $(S_t)$ is a $C_0$-semigroup on $C_V(\RR_+, \RR)$.  
\end{lemma}

\begin{proof}
    It is clear that $S_0$ is the identity.  The semigroup property holds
    because, for $s, t \geq 0$, we have
    %
    \begin{equation*}
        S_{s + t} f = f(\cdot + s + t) = S_t \, S_s f
    \end{equation*}
    %
    Regarding continuity, fix $f \in C_V(\RR_+, \RR)$ and let $(t_n)$ be a real sequence
...
Let $C_0^1(\RR_+, \RR)$ be the set of all continuously differentiable $f \in
C_V(\RR_+, \RR)$ with $f' \in C_V(\RR+, \RR)$.  The set $C_0^1(\RR_+, \RR)$ is paired
with the norm $\|f\| = \sup_x |f(x)| + \sup_x |f'(x)|$.  

\begin{lemma}\label{l:ltsemi2}
    $(S_t)$ is a $C_0$-semigroup on $C_0^1(\RR_+, \RR)$.  
\end{lemma}

\begin{proof}
    In view of Lemma~\ref{l:ltsemi}, we only need to check continuity.
    Fixing $f \in C_0^1(\RR_+, \RR)$, we have
    %
    \begin{equation*}
        \| S_t f - f \|
    ...
    \end{equation*}
    %
    Since $f$ and $f'$ are both in $C_V(\RR_+, \RR)$, the proof of
...
Let $C_K(\RR)$ be the set of all continuous real-valued functions $f$ on
...
\begin{proof}
    ...
    $\|S_t\|$ is bounded in $t$.  Since $C_K(\RR)$ is dense in $L_1(\RR)$ under this
    ...
    To this end, fix $f \in C_K(\RR)$ and let $K$ be a compact set such that $f$
...
The map $A$ is called the \navy{infinitesimal generator} (or just
\navy{generator}) of $(S_t)$ and $\dD(A)$
is its domain.  We sometimes also refer to the pair $(A, \dD(A))$ as the
generator.

\begin{lemma}\label{l:dals}
    The set $\dD(A)$ is a linear subspace of $E$ and $A$ is a closed linear
    operator from $\dD(A)$ to $E$.
\end{lemma}
...
\begin{proof}
    ...
    This proves that $A$ is a linear operator from linear subspace $\dD(A)$ to
    $E$.  The proof that $A$ is closed can be found in Theorem~1.3.16 of
...
\begin{proof}
    Fix $u \in E$ and $t \geq 0$.
    For $h > 0$, the properties of the integral described in
    Lemma~\ref{l:riemann} imply that
    %
    \begin{align*}
    ...
    \downarrow 0$ gives $A \int_0^t S_s \, u \diff s = S_t u - u$. Hence both
    claims in \eqref{eq:pathfrome} hold.
\end{proof}
...
\begin{proposition}\label{p:sgufg}
    Let $(S_t)$ and $(T_t)$ be two $C_0$-semigroups on $E$ with 
    generators $(A, \dD(A))$ and $(B, \dD(B))$.  If $(A, \dD(A)) = (B, \dD(B))$,
    then $(S_t) = (T_t)$.
\end{proposition}

\begin{proof}
    Let the statement hold, so that $(S_t)$ and $(T_t)$ are two $C_0$-semigroups
    with generator $A$ on domain $\dD(A)$. Fix $t > 0$.  We claim
    that $S_t = T_t$ on $E$. Since $\dD(A)$ is dense in $E$, it suffices to show
    that $S_t$ and $T_t$ agree on
    $\dD(A)$.
...
\begin{theorem}\label{t:ivps}
    Let $(S_t)$ be a $C_0$-semigroup with generator $(A, \dD(A))$
...
Let $(A, \dD(A))$ be the generator of a $C_0$-semigroup $(S_t)$ on $E$.
Recalling that $A$ is a closed linear operator (Lemma~\ref{l:dals}), the
\navy{resolvent} of $A$ is, for each $\lambda\in\rho(A)$, the bounded and
bijective linear operator
%
\begin{equation*}
   R_{\lambda}(A)=(\lambda - A)^{-1} 
\end{equation*}
%
from $E$ to $\dD(A)$.  We also refer to $R_\lambda(A)$ as the resolvent of the
semigroup $(S_t)$.  We write $R_\lambda$ instead of $R_\lambda(A)$ when $A$ is
understood.

\begin{proposition} 
    Let $A$ be the generator of a $C_0$-semigroup $(S_t)$
    satisfying $\| S_t\|\leq M e^{at}$ for all $t\geq 0$.  
    If $\lambda\in \CC$ with $\Re(\lambda)>a$, then 
    $\lambda$ is in the resolvent set $\rho(A)$.  Moreover,
    %
	\begin{enumerate}
		\item the operator norm of the resolvent obeys 
            %
            \begin{equation*}
                \|R_{\lambda}\|\leq\frac{M}{\Re(\lambda)-a}
                \quad \text{and}
            \end{equation*}
            %
		\item the resolvent has the integral representation
            %
            \begin{equation*}
                R_{\lambda}
                = \int_{0}^{\infty} e^{-\lambda t}S_t \diff t.
            \end{equation*}
	        %	
	\end{enumerate}
    %
...
\begin{proof}
    Let $(S_t)$ be as stated, with $\|S_t\| \leq M \exp(ta)$ for all $t \geq 0$.
    Fix $\lambda \in \CC$ with $\Re(\lambda) > a$. Let
    $T_{\lambda}=\int_{0}^{\infty} e^{-\lambda t}S_t \diff t$.
    Fixing $u\in E$, we have
    %
    %
    \begin{equation*}
        \|T_{\lambda}u\|
        = \left\|
                \int_0^\infty \me^{-\lambda t} S_tu \diff t 
            \right\|
            \leq \int_{0}^{\infty} |e^{-\lambda t} | \cdot \|S_tu\| \diff t
    \end{equation*}
    %
    Since $|\me^{-\lambda t}| = |\me^{- \Re(\lambda) t}|$, we obtain
    %
    \begin{equation*}
        \|T_{\lambda}u\|
        \leq  \|u\| M \int_0^\infty \me^{(a-\Re(\lambda)) t} \diff   t
        = \frac{M}{\Re(\lambda)-a} \|u\| .
    \end{equation*}
    %
    This proves that $T_{\lambda}$ is well-defined and bounded on $E$, and that
    (i) holds.  Next we claim that
	$T_{\lambda}(\lambda - A)  = (\lambda - A)T_{\lambda} = I$,
    ...
    \begin{align*}
    	T_{\lambda} Au
    	& = \int_{0}^{\infty} \me^{-\lambda t}S_t \lim_{h \downarrow 0}
            \frac{S_h-I}{h}u\diff t
        \\
    	& = \lim_{h \downarrow 0}\int_{0}^{\infty} \me^{-\lambda t}S_t 
            \frac{S_h-I}{h}u\diff t
    	= \lim_{h \downarrow 0}\frac{S_h-I}{h}
            \int_0^\infty \me^{-\lambda t}S_t u\diff t = AT_{\lambda}u
    \end{align*}
    %
    This establishes the first equality in \eqref{eq:rvt}.  Regarding the
    second, observe that
    %
    \begin{align*}
       AT_{\lambda}u &
        = \lim_{h \downarrow 0}\frac{S_h-I}{h}
            \int_{0}^{\infty} \me^{-\lambda t}S_t u\diff t 
            = \lim_{h \downarrow 0}
            (\frac{1}{h}\int_{0}^{\infty} \me^{-\lambda t}S_{t+h} u\diff t)
            - \frac{1}{h}\int_{0}^{\infty} \me^{-\lambda t}S_t u\diff t)
        \\
       & = \lim_{h \downarrow 0}(\frac{1}{h}\int_{h}^{\infty} \me^{-\lambda( t-h)}S_{t} u\diff t)
         -\frac{1}{h}\int_{0}^{\infty} \me^{-\lambda t}S_t u\diff t)\\
       & = \lim_{h \downarrow 0}- \me^{\lambda h}\frac{1}{h}\int_{0}^{h} \me^{-\lambda t}S_t u\diff t
            +\lim_{h \downarrow 0}\frac{1}{h}(\me^{\lambda h}-1)
            \int_0^\infty \me^{-\lambda t}S_t u\diff t 
            = -u+\lambda T_{\lambda}u
    \end{align*}
    %
    It follows that $\lambda$ is in the resolvent set $\rho(A)$ and that the
    integral representation in (ii) holds.
\end{proof}



\subsection{Example: Diffusion Semigroups} 

Consider the Banach
Space $E = C[0,1]$ and the differential operator 
%
\begin{equation*}
Af:=f''
\end{equation*}  
