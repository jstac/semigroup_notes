...
Also, given $\Xsf \subset \RR$ and normed linear space $F$,
%
\begin{itemize}
    \item $C_0(\Xsf, F)$ is the set of continuous functions from $\Xsf$ to $F$
        and
    \item $C_1(\Xsf, F)$ is the set of continuously differentiable functions
        from $\Xsf$ to $F$.
\end{itemize}
%
...
\begin{lemma}\label{l:expcom2}
    Given $A \in \lL(E)$, the function $f(t) \coloneq \exp(tA)$ is
...
\begin{proof}
    ...
    0$ gives $\dot f(t) = f(t) A $.  A small variation on the argument 
    shows that $\dot f(t) = A f(t)$ also holds.
\end{proof}
