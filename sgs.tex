...
an \navy{algebraic operator (AO) semigroup}.  If, in addition, $E$ is an Banach
...
Let $u$ be a map from $\RR_+$ to $E$, where $E$ is a Banach space. If, for some
$t > 0$, the limit
%
\begin{equation*}
    \dot u(t)
...
\begin{example}
    Consider the exponential function $e(t) = \exp(t A)$ from \S\ref{ss:ucsemi},
    where $t \in \RR_+$ and $A \in \lL(E)$.  We have
    %
    \begin{equation*}
        \frac{e(t + h) - e(t)}{h}
    ...
    0$ gives $\dot e(t) = \exp(tA) A $.  A small variation on the argument above
    shows that $\dot e(t) = A \exp(tA)$ also holds.
\end{example}

Let $u$ be a continuous map from $[a, b]$ to $E$.  We call $I$ the
(Riemann) \navy{integral} of $u$ over $[a,b]$ and write $I = \int_a^b u(s) \diff s$ if,
...
\end{equation*}
%
When $u$ is continuous on $\RR_+$ and $a, b \in \RR_+$ with $a < b$, the
integral has the following elementary properties:
%
\begin{enumerate}
    ...
    \item $\| \int_a^b u(s) \diff s \| \leq \int_a^b \|
        u(s)  \| \diff s$.
    \item $u(t) = \lim_{h \to 0} \, (1/h) \int_t^{t+h} u(s) \diff s$ for all $t \in \RR_+$.
\end{enumerate}
...
\begin{lemma}\label{l:diffpath}
    If $(S_t)$ is a $C_0$-semigroup, $u_0 \in E$ and $u_t = S_t u_0$
    for all $t \geq 0$, then, for all $t \geq 0$,
    %
    \begin{equation*}
...
\begin{proof}
    Fix $u_0 \in E$ and let $u_t$ be as defined in the lemma.  
\end{proof}
...
\begin{theorem}\label{t:diffpath}
    ...
    \begin{enumerate}
        \item $u_t \in \dD(A)$,
        \item the derivative $\dot u_t$ exists, and $\dot u_t = A u_t$.
    \end{enumerate}
    %
\end{theorem}

\begin{proof}
    Fix $u_0 \in \dD(A)$ and let $u_t = S_t u_0$.
    Claim (i) follows from (ii) because, when $u_t$ is differentiable, 
    %
    \begin{equation*}
         \frac{S_h u_t - u_t}{h}
         = \frac{u_{t+h} - u_t}{h} \to \dot u_t \quad \text{as } h \downarrow 0.
    \end{equation*}
    %
...
\begin{theorem}\label{t:dense}
    If $(S_t)$ is a $C_0$-semigroup, then $\dD(A)$ is dense in $E$.
\end{theorem}

\begin{proof}
    To see that $\dD(A)$ is dense, fix $u_0 \in E$, set $u_t = S_t u_0$
    and let $v_h = (1/h) \int_0^h u_s \diff s$ for all $h > 0$. By the results
    in \S\ref{ss:prel}, we have $v_h \to u_0$ as $h \downarrow 0$.  Moreover, by
    XXX, $v_h$ is in $\dD(A)$ for all $h > 0$.   Hence $\dD(A)$ is dense in $E$.
\end{proof}
